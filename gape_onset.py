...
first_gapes_dir = os.path.join(results_dir,'first_gapes')
if os.path.isdir(first_gapes_dir) == False:
	os.mkdir(first_gapes_dir)
	...
	for time in range(sac_gapes_train2_boolean.shape[1]):
		...
		if this_data == 0 and previous_data == 1:
			...
			if this_start > 0 and this_start <2000 and this_end - this_start >= 500:
				first_gapes_sac_train2.append(this_save_inds)
				break

sac_gape_array_train2 = np.array(first_gapes_sac_train2)
...
se_sac_train2 = sd_mean_gape_onset_sac_train2/np.sqrt(len(first_gapes_sac_train2[1]))
...
np.save('/media/cmazzio/large_data/CM26/CM26_CTATrain2_sac_h2o_230818_104128/gape_onset_plots/CM26_CTATrain2_first_sac_gapes.npy', np.array(first_gapes_sac_train2))
		...
		for time in range(qhcl_gapes_boolean.shape[1]):
			...
			if this_data == 0 and previous_data == 1:
				...
				if this_start < 2000 and this_start > 100 and this_end - this_start >= 300:
					first_gapes_qhcl.append(this_save_inds)
					break
				
	np.save('media/cmazzio/large_data/CM26/CM26_Test2_h2o_sac_ca_qhcl_230820_121050/gape_onset_plots/CM26_Test2_h2o_sac_ca_qhcl_gapes_qhcl', first_gapes_qhcl)
	...
	for trial in range(sac_gapes_boolean.shape[0]):
		for time in range(sac_gapes_boolean.shape[1]):
			...
			if this_data == 0 and previous_data == 1:
				...
				this_save_inds = [trial, this_start, this_end]
				if this_start < 2000 and this_start > 100 and this_end - this_start >= 300:
					first_gapes_sac.append(this_save_inds)
					break
	
	np.save('media/cmazzio/large_data/CM26/CM26_Test2_h2o_sac_ca_qhcl_230820_121050/gape_onset_plots/CM26_Test2_h2o_sac_ca_qhcl_gapes_sac', first_gapes_sac)
		
				
	mean_gape_onset_qhcl =  np.mean(first_gapes_qhcl[1], axis=0)
	mean_gape_onset_sac = np.mean(first_gapes_sac[1], axis=0)
	
	sd_mean_gape_onset_qhcl = np.std(first_gapes_qhcl[1], axis=0)
	sd_mean_gape_onset_sac = np.std(first_gapes_sac[1], axis = 0)
	
	se_qhcl = sd_mean_gape_onset_qhcl/np.sqrt(len(first_gapes_qhcl[1]))
	se_sac = sd_mean_gape_onset_sac/np.sqrt(len(first_gapes_sac[1]))
