...
try:
    ...
    try:
        # (num_tastes,num_trials,time) #2000 pre, 5000 post
        emg_filt = np.load(os.path.join(
            data_dir, 'emg_output', 'emg', 'emg_filt.npy'))
        ...
        if bool_val == 'y':
            emg_filt_data_dir = easygui.diropenbox(
                title='\tPlease select the folder where emg_filt.npy is stored.')
    ...
    try:
        # (num_tastes,num_trials,time) #2000 pre, 5000 post
        env = np.load(os.path.join(
            data_dir, 'emg_output', 'emg', 'emg_env.npy'))
        ...
        if bool_val == 'y':
            env_data_dir = easygui.diropenbox(
                title='\tPlease select the folder where env.npy is stored.')
    ...
    try:
        all_taste_gapes = np.load(os.path.join(
            results_dir, 'emg_clust_results.npy'))
...
if prev_run == 0:
    ...
    # Calculate pre-taste amplitude threshold across all trials for this taste
        ...
        for tr_i in tqdm.tqdm(range(max_num_trials)):
            # Make sure a delivery actually happened - nan otherwise
            if not np.isnan(emg_filt[t_i, tr_i, 0]):
                f, ax = plt.subplots(nrows=5, ncols=2, figsize=(10, 10))
                gs = ax[4, 0].get_gridspec()
                # pre_taste+post_taste length array
                tr_emg = emg_filt[t_i, tr_i, :].flatten()
                # Plot raw EMG data
                ...
                [peak_inds, peak_props] = find_peaks(
                    tr_env-mu_env, prominence=sig_env, distance=min_inter_peak_dist, width=0, rel_height=0.99)
                # ___Find edges of peaks using peak widths function
                # This is half-height width so double for ~full width
                [peak_ws, _, _, _] = peak_widths(
                    tr_env-mu_env, peak_inds, rel_height=0.5)
                peak_left = np.array(
                    [np.max((peak_inds[p_i] - peak_ws[p_i], 0)) for p_i in range(len(peak_inds))])
                peak_right = np.array([np.min(
                    (peak_inds[p_i] + peak_ws[p_i], pre_taste+post_taste-1)) for p_i in range(len(peak_inds))])
                # ___Find frequency using FFT on 3-peak fit
                # Store instantaneous fft frequency
                peak_freq_fft = np.zeros(len(peak_inds))
                for p_i in np.arange(1, len(peak_inds)-1):
                    ...
                    num_repeat = np.ceil(
                        1000/np.shape(repeat_peak)[1]).astype('int')
                    for rp_i in range(num_repeat):
                        if np.mod(rp_i, 2) == 0:
                            repeat_peak = np.concatenate(
                                (repeat_peak, np.fliplr(peak_form)), 1)
                        else:
                            repeat_peak = np.concatenate(
                                (repeat_peak, peak_form), 1)
                ...
                peak_freq = np.zeros(len(peak_inds))
                peak_amp = np.zeros(len(peak_inds))
                jit = np.random.rand()
                for i in range(len(peak_inds)):
                    p_i = peak_inds[i]
                    ax[1, 0].axvline(p_i-pre_taste, color='k',
                                     linestyle='dashed', alpha=0.5)
                    # w_i = peak_props['widths'][i]
                    w_i = peak_right[i] - peak_left[i]
                    # Calculate instantaneous frequency
                    peak_freq[i] = 1/(w_i/1000)
                    w_h = peak_props['width_heights'][i]
                    if np.mod(i, 2) == 0:
                        ax[1, 0].plot([peak_left[i]-pre_taste, peak_right[i]-pre_taste],
                                      [w_h + jit, w_h + jit], color='g', linestyle='dashed', alpha=0.5)
                    else:
                        ax[1, 0].plot([peak_left[i]-pre_taste, peak_right[i]-pre_taste],
                                      [w_h, w_h], color='r', linestyle='dashed', alpha=0.5)
                ...
                for gpi in range(len(gape_peak_inds)):
                    x_vals = np.arange(
                        gape_starts[gpi], gape_ends[gpi]) - pre_taste
                    ax[2, 1].fill_between(
                ...
                for gtf in range(len(gape_times_fft)):
                    taste_gapes[tr_i, gape_starts_fft[gtf]
                        :gape_ends_fft[gtf]] = 1
                ax[3, 0].plot(peak_inds-pre_taste, peak_freq_fft)
                for gtf in gape_times_fft:
                    ax[3, 0].scatter(
                        gtf-pre_taste, (max_gape_band+min_gape_band)/2, marker='*', color='k', alpha=0.5)
                ...
                for gpi in range(len(gape_peak_inds_fft)):
                    x_vals = np.arange(
                        gape_starts_fft[gpi], gape_ends_fft[gpi]) - pre_taste
                    ax[3, 1].fill_between(
                        x_vals, min_env*np.ones(len(x_vals)), max_env*np.ones(len(x_vals)), color='r', alpha=0.2)
                ...
                axbig.plot(np.arange(0, pre_taste),
                           tr_env[pre_taste:pre_taste+pre_taste])
                for gpi in range(len(gape_times_fft)):
                    if (gape_times_fft[gpi] >= pre_taste)*(gape_times_fft[gpi] <= pre_taste+pre_taste):
                        ...
                        axbig.fill_between(
                            x_vals, min_env*np.ones(len(x_vals)), max_env*np.ones(len(x_vals)), color='r', alpha=0.2)
                        axbig.axvline(
                            gape_times_fft[gpi]-pre_taste, linestyle='dashed', color='k', alpha=0.2)
                # Clean up and save
                f.tight_layout()
                f.savefig(os.path.join(taste_save_dir,
                          'emg_gapes_trial_' + str(tr_i) + '.png'))
                f.savefig(os.path.join(taste_save_dir,
                          'emg_gapes_trial_' + str(tr_i) + '.svg'))
                plt.close(f)
...
# Store waveform start indices in 'all_taste_gapes'
gape_all_taste_starts = []
#Store waveform end indices in 'all_taste_gapes'
gape_all_taste_ends = []
    ...
    for tr_i in range(max_num_trials):
        ...
        for gs in gape_starts:
            try:
                ...
                gape_all_taste_starts.append([t_i,tr_i,gs])
                gape_all_taste_ends.append([t_i,tr_i,ge])
            except:
                pass
            gape_tastes.extend([t_i])
            # store in ms from taste delivery
            gape_start_times.extend([gs-pre_taste])
...
gape_start_times_group_options = np.arange(np.floor(gape_start_times_min/100).astype('int')*100, np.ceil(
    gape_start_times_max/100).astype('int')*100, 100)  # Step from min to max in 100 ms bins
gape_start_times_group_labels = gape_start_times_group_options/100
gape_start_times_group = np.array([np.argmin(
    np.abs(gst-gape_start_times_group_options)) for gst in gape_start_times])
gape_start_times_labels = [gape_start_times_group_labels[gstg]
                           for gstg in gape_start_times_group]
...
for nc in range(env_norm_width_n_clusters):
    bool_val = bool_input("Is cluster " + str(nc) +
                          " composed of true gapes? ")
    if bool_val == 'y':
...
kept_all_taste_gapes = np.zeros(np.shape(all_taste_gapes))
for kgi in np.where(keep_gape_inds)[0]:
    atg_t, atg_tr, atg_st = gape_all_taste_starts[kgi]
    _, _, atg_e = gape_all_taste_ends[kgi]
    kept_all_taste_gapes[atg_t, atg_tr, atg_st:atg_e] = 1
np.save(os.path.join(results_dir, 'emg_clust_results.npy'),
        np.array(kept_all_taste_gapes))
