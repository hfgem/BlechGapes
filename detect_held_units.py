...
electrode_same_bool = ((all_inter_electrode_dist==0).astype('int')).squeeze()
...
if use_electrode == 'y':
    keep_inds = np.where((electrode_same_bool)*(joint_norm_score <= score_cutoff))[0]
else:
    keep_inds = np.where(joint_norm_score <= score_cutoff)[0]
neur_pairs_low_norm_score = np.array(all_neur_combos)[keep_inds]
neur_pair_norm_scores = joint_norm_score[keep_inds]
